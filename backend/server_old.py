    ...
    initial_projects = [
        {
            ...
            "link": "https://appportal-2.preview.emergentagent.com",
            "featured": True,
            "order": 1,
...
# =============================================================================
# TRIVOR CONTACTS - ARCHIVIO CONTATTI CONDIVISO
# =============================================================================
...
class ContactCreate(BaseModel):
    ...
    email: str = ""
    telefono: str = ""
    whatsapp: str = ""  # Può essere diverso dal telefono
    azienda: str = ""
    ruolo: str = ""
    indirizzo: str = ""
    note: str = ""
    gruppi: List[str] = []  # IDs dei gruppi
    preferito: bool = False
    avatar_color: str = ""  # Colore per avatar generato
...
class GroupCreate(BaseModel):
    nome: str
    descrizione: str = ""
    colore: str = "#3B82F6"  # Default blue
...
@api_router.post("/contacts/{contact_id}/toggle-preferito")
...
@api_router.get("/contacts/groups/list")
async def get_groups(username: str = Depends(verify_trivordoc_credentials)):
    ...
    for group in groups:
        count = await db.trivor_contacts.count_documents({"gruppi": group["id"]})
        group["membri_count"] = count
    
    return groups

@api_router.get("/contacts/groups/{group_id}")
async def get_group(group_id: str, username: str = Depends(verify_trivordoc_credentials)):
    """Get single group with members"""
    group = await db.trivor_groups.find_one({"id": group_id}, {"_id": 0})
    if not group:
        raise HTTPException(status_code=404, detail="Gruppo non trovato")
    
    # Get members
    members = await db.trivor_contacts.find({"gruppi": group_id}, {"_id": 0}).to_list(500)
    group["membri"] = members
    group["membri_count"] = len(members)
    
    return group

@api_router.post("/contacts/groups")
...
@api_router.put("/contacts/groups/{group_id}")
...
@api_router.delete("/contacts/groups/{group_id}")
...
async def get_contacts_stats(username: str = Depends(verify_trivordoc_credentials)):
    """Get contacts statistics"""
    total = await db.trivor_contacts.count_documents({})
    preferiti = await db.trivor_contacts.count_documents({"preferito": True})
    gruppi = await db.trivor_groups.count_documents({})
    
    # Contacts with email
    with_email = await db.trivor_contacts.count_documents({"email": {"$ne": ""}})
    # Contacts with WhatsApp
    with_whatsapp = await db.trivor_contacts.count_documents({"whatsapp": {"$ne": ""}})
    
    # Recent contacts
    recent = await db.trivor_contacts.find({}, {"_id": 0}).sort("created_at", -1).limit(5).to_list(5)
    
    return {
        "totale_contatti": total,
        "preferiti": preferiti,
        "gruppi": gruppi,
        "con_email": with_email,
        "con_whatsapp": with_whatsapp,
        "recenti": recent
    }
