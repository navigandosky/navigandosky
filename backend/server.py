...
FRONTEND_URL = os.environ.get('FRONTEND_URL', 'https://smartdomo-bugfix.preview.emergentagent.com')
...
async def create_user(data: UserCreate, token: str = Query(...)):
    ...
    await db.property_config.insert_one(empty_property_config)
        ...
        if ewelink_devices and ewelink_devices.get("devices"):
            for device in ewelink_devices.get("devices", []):
                if device.get("id") == device_id:
                    ...
                    # Daily and Monthly kWh - eWeLink S60TPF sends values x100
                    # e.g., 24 raw = 0.24 kWh actual, 1112 raw = 11.12 kWh
                    if params.get("dayKwh") is not None:
                        daily_kwh = float(params.get("dayKwh", 0)) / 100
                        consumption["daily_kwh"] = round(daily_kwh, 2)
                        consumption["has_consumption_data"] = True
                    
                    if params.get("monthKwh") is not None:
                        monthly_kwh = float(params.get("monthKwh", 0)) / 100
...
async def balin_api_request(path: str, method: str = "GET", user_id: str = None, params: Dict = None) -> Dict:
    """Make authenticated request to Balin API"""
    creds = await get_balin_credentials(user_id)
    if not creds:
...
async def balin_status(token: Optional[str] = None):
    ...
    try:
        ...
        devices = await balin_api_request("/devices", user_id=user["id"])
        return {
            "connected": True,
...
async def get_balin_devices(token: Optional[str] = None):
    ...
    try:
        ...
        # Enrich device data with formatted information
        enriched_devices = []
        for device in devices:
...
@api_router.get("/balin/device/{imei}")
async def get_balin_device(imei: str, token: Optional[str] = None):
    ...
    try:
        device = await balin_api_request(f"/device/{imei}", user_id=user["id"])
...
@api_router.get("/balin/device/{imei}/history")
async def get_balin_device_history(
    imei: str, 
    start: int,  # timestamp in ms
    stop: int,   # timestamp in ms
    token: Optional[str] = None,
    skip: int = 0,
    limit: int = 1500,
    ...
    try:
        result = await balin_api_request(f"/positionsHistory/{imei}", user_id=user["id"], params=params)
        return result
    except HTTPException:
        raise
    except Exception as e:
...
@api_router.get("/balin/device/{imei}/trips")
async def get_balin_device_trips(
    ...
    try:
        result = await balin_api_request(f"/tripPositionsHistory/{imei}", user_id=user["id"], params=params)
        return result
    except HTTPException:
        raise
    except Exception as e:
