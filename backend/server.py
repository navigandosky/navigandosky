...
FRONTEND_URL = os.environ.get('FRONTEND_URL', 'https://buildmaster-47.preview.emergentagent.com')
...
async def cerca_centri_assistenza_web(
    ...
    try:
        ...
        # Usa DuckDuckGo per la ricerca (gratuito, senza API key)
        ...
        async with httpx.AsyncClient(timeout=15.0) as client:
            ...
            if response.status_code != 200:
                raise HTTPException(status_code=500, detail="Errore nella ricerca web")
            
            # Parse HTML response
            html = response.text
            results = []
            
            # Estrai risultati (semplificato - cerca pattern comuni)
            import re
            
            # Pattern per estrarre titoli e snippet
            ...
            for i, (url, title) in enumerate(titles[:10]):
                result = {
                    "titolo": title.strip(),
                    "url": url,
                    "descrizione": snippets[i].strip() if i < len(snippets) else "",
                    "marca": marca,
                    "localita": localita
                }
                
                # Prova a estrarre telefono dalla descrizione
                phone_match = re.search(r'(\+?39)?[\s.-]?(\d{2,4})[\s.-]?(\d{5,8})', snippets[i] if i < len(snippets) else "")
                if phone_match:
                    result["telefono_trovato"] = phone_match.group(0)
                
                results.append(result)
            
            # Se DuckDuckGo non restituisce risultati formattati, prova altro approccio
            if not results:
                # Restituisci link di ricerca diretti
                results = [
                    {
                        "titolo": f"Cerca su Google: Centro Assistenza {marca} {localita}",
                        "url": f"https://www.google.com/search?q=centro+assistenza+{quote(marca)}+{quote(localita)}",
                    ...
                    {
                        "titolo": f"Sito ufficiale {marca} - Assistenza",
                        "url": f"https://www.google.com/search?q={quote(marca)}+assistenza+clienti+italia",
                        "descrizione": f"Trova il sito ufficiale {marca} per l'assistenza clienti",
                        "marca": marca,
                        "localita": localita,
                        "tipo": "link_ricerca"
                    },
                    {
                        "titolo": f"Pagine Gialle - {marca} {localita}",
                        "url": f"https://www.paginegialle.it/ricerca/{quote(marca)}/assistenza/{quote(localita)}",
                        "descrizione": f"Cerca su Pagine Gialle centri assistenza {marca}",
                        "marca": marca,
                        "localita": localita,
                        "tipo": "link_ricerca"
        ...
        return {
            ...
            "risultati": results,
            "totale": len(results),
            "suggerimento": f"Cerca 'centro assistenza {marca} {localita}' per più risultati"
        }
        
    except httpx.TimeoutException:
        # In caso di timeout, restituisci link di ricerca diretti
        return {
            "query": f"{marca} {localita}",
            "risultati": [
                {
            ...
            "totale": 1,
            "nota": "Timeout nella ricerca, usa i link suggeriti"
        }
    except Exception as e:
        logger.error(f"Errore ricerca centri assistenza: {e}")
        raise HTTPException(status_code=500, detail=f"Errore nella ricerca: {str(e)}")
